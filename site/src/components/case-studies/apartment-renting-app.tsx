          ...
          <div style={{ fontFamily: "var(--font-body)" }}>
                                                  ...
                                                  <Reveal delay={140} style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, maxWidth: 560 }}>
                                                    {heroStats.map((s) => (
                                                        <div key={s.label} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "20px 18px" }}>
                                                                          <div style={{ fontFamily: "var(--font-heading)", fontWeight: 800, fontSize: 28, color: ACCENT, marginBottom: 8 }}>{s.value}</div>div>
                                                                          <div style={{ fontSize: 12.5, color: TEXT_MUTED, lineHeight: 1.4 }}>{s.label}</div>div>
                                                        </div>div>
                                                      ))}
                                                  </Reveal>Reveal>
                                      </div>div>
            ...
            { label: "Tools", value: "Figma · ProtoPie · FigJam" },
                                                ...
                                                <div key={d.label} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "14px 18px" }}>
                                                                    <div style={{ fontSize: 11, color: TEXT_MUTED, marginBottom: 4, letterSpacing: "0.1em", textTransform: "uppercase" }}>{d.label}</div>div>
                                                                    <div style={{ fontSize: 14, color: TEXT, fontWeight: 500 }}>{d.value}</div>div>
                                                </div>div>
                                              ))}
                                                            </div>div>
                                              </div>div>
                          ...
                          {/* 02 Problem */}
                            ...
                            <div key={p.stat} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "28px 24px" }}>
                                              <div style={{ fontFamily: "var(--font-heading)", fontWeight: 800, fontSize: 40, color: ACCENT, marginBottom: 12, lineHeight: 1 }}>{p.stat}</div>div>
                                              <div style={{ fontSize: 14, color: TEXT_MUTED, lineHeight: 1.6 }}>{p.body}</div>div>
                            </div>div>
                          ))}
                                              </div>div>
                                              <div style={{ background: "rgba(61,114,180,0.06)", border: "1.5px solid rgba(61,114,180,0.2)", borderRadius: 16, padding: "28px 32px" }}>
                                                            <p style={{ fontSize: 17, color: TEXT, lineHeight: 1.7, margin: 0 }}>
                                                                            <strong style={{ color: ACCENT }}>How might we</strong>strong> design a rental marketplace that
                          ...
                          {/* 03 Objectives */}
                            ...
                            <div key={o.title} className="hover-card" style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 24 }}>
                                              <div style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 16, color: ACCENT, marginBottom: 8 }}>{o.title}</div>div>
                                              <div style={{ fontSize: 14, color: TEXT_MUTED, lineHeight: 1.6 }}>{o.body}</div>div>
                            </div>div>
                          ))}
                                              </div>div>
                                  </Reveal>Reveal>
                          {/* 04 Research */}
                                  <Reveal style={{ marginBottom: 96 }}>
                                              ...
                                              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 48, alignItems: "start" }}>
                                                            <div>
                                                                            <h3 style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 22, color: TEXT, marginBottom: 12 }}>Methods</h3>h3>
            ...
            { insight: "\"By the time I call back, someone else has already taken it.\"", theme: "Scheduling lag causes renter drop-off and lost landlord revenue" },
                                                ...
                                                <div key={q.theme} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 24, borderLeft: `4px solid ${ACCENT}` }}>
                                                                    <p style={{ fontFamily: "var(--font-heading)", fontStyle: "italic", fontSize: 15, color: TEXT, marginBottom: 10, lineHeight: 1.5 }}>{q.insight}</p>p>
                                                                    <p style={{ fontSize: 12, color: TEXT_MUTED, margin: 0, letterSpacing: "0.04em" }}>→ {q.theme}</p>p>
                                                </div>div>
                                              ))}
                                                            </div>div>
                                              </div>div>
            ...
            { phase: "Book", icon: "✅", steps: ["Schedule viewing", "Sign agreement", "Pay deposit"], pain: "Multi-step handoffs cause drop-off" },
                                            ...
                                            <div key={p.phase} style={{ padding: 24, background: i % 2 === 0 ? CARD : NAV_BG, borderRight: i < 3 ? `1px solid ${BORDER}` : undefined }}>
                                                              <div style={{ fontSize: 28, marginBottom: 12 }}>{p.icon}</div>div>
                                                              <div style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 16, color: ACCENT, marginBottom: 14 }}>{p.phase}</div>div>
                                                              <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
                                                                {p.steps.map((s) => <div key={s} style={{ fontSize: 13, color: TEXT, lineHeight: 1.4 }}>{s}</div>div>)}
                                                              </div>div>
                          ...
                          {/* 06 Key Screens */}
                            ...
                            <div key={s.title} className="hover-card" style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 24 }}>
                                              <div style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 15, color: ACCENT, marginBottom: 8 }}>{s.title}</div>div>
                                              <div style={{ fontSize: 13.5, color: TEXT_MUTED, lineHeight: 1.6 }}>{s.body}</div>div>
                            </div>div>
                          ))}
                                              </div>div>
                          ...
                          {/* 08 Design System */}
                                  <Reveal style={{ marginBottom: 96 }}>
                                              ...
                                              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "start" }}>
                                                            <div>
                                                                            <h3 style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 20, color: TEXT, marginBottom: 20 }}>Colour Tokens</h3>h3>
                                ...
                                <div key={t.label} style={{ display: "flex", alignItems: "center", gap: 14, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: "12px 16px" }}>
                                                      ...
                                                      <div>
                                                                              <div style={{ fontSize: 13, fontWeight: 600, color: TEXT }}>{t.label}</div>div>
                                                                              <div style={{ fontSize: 11, color: TEXT_MUTED, fontFamily: "var(--font-mono)" }}>{t.hex}</div>div>
                                                      </div>div>
                                </div>div>
                              ))}
                                                                            </div>div>
                                                            ...
                                                            <div>
                                                                            <h3 style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 20, color: TEXT, marginBottom: 20 }}>Typography Scale</h3>h3>
            ...
            { name: "Label", size: "11–12px", weight: "500", sample: "MOVE-IN DATE · 3 BHK · FURNISHED" },
                          ...
                          {/* 09 Outcomes */}
                            ...
                            <div key={o.label} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "28px 28px" }}>
                                              <div style={{ fontFamily: "var(--font-heading)", fontWeight: 800, fontSize: 48, color: ACCENT, marginBottom: 12, lineHeight: 1 }}>{o.value}</div>div>
                                              <div style={{ fontSize: 14, color: TEXT_MUTED, lineHeight: 1.6 }}>{o.label}</div>div>
                            </div>div>
                          ))}
                                              </div>div>
                                              <div style={{ background: TEXT, borderRadius: 20, padding: "48px 56px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 32 }}>
                                                            <div>
                                                                            <div style={{ fontFamily: "var(--font-heading)", fontWeight: 800, fontSize: 28, color: "white", marginBottom: 10 }}>See the full case study</div>div>
